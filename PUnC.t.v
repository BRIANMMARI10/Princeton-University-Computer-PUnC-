...
module PUnCTATest;
	...
	initial begin
		...
		`START_TEST("quotient1");
		`WAIT_PC_FREEZE;
		`ASSERT_REG_EQ(6, 16'd5); //quotient
		`ASSERT_MEM_EQ(27, 16'd5);
		`ASSERT_REG_EQ(5, 16'd0); //remainder
		`ASSERT_MEM_EQ(28, 16'd0);
		`ASSERT_REG_EQ(4, 16'b1111111111111111); //exactly guess
		`ASSERT_MEM_EQ(29, 16'b1111111111111111); 

		`START_TEST("quotient2");
		`WAIT_PC_FREEZE;
		`ASSERT_REG_EQ(6, 16'd206); //quotient
		`ASSERT_MEM_EQ(27, 16'd206);
		`ASSERT_REG_EQ(5, 16'd1); //remainder
		`ASSERT_MEM_EQ(28, 16'd1);
		`ASSERT_REG_EQ(4, 16'd0);//not exactly guess
		`ASSERT_MEM_EQ(29, 16'd0); 

		`START_TEST("quotient3");
		`WAIT_PC_FREEZE;
		`ASSERT_REG_EQ(6, 16'd206); //quotient
		`ASSERT_MEM_EQ(28, 16'd206);
		`ASSERT_REG_EQ(5, 16'd1); //remainder
		`ASSERT_MEM_EQ(29, 16'd1);
		`ASSERT_REG_EQ(4, 16'd0);//not exactly guess
		`ASSERT_MEM_EQ(30, 16'd0); 
